...
void pbf_init(pbf p, int w, int h, int mode, bool share)
{
	...
	if (p) {
		...
		if (!globalGL.wglChoosePixelFormatARB(
				dc, iattribs, fattribs, MAX_PFORMATS, pformat, &nformats))
			return;
		format = pformat[0];

		iattribs[0] = 0;
		p->pBuffer = globalGL.wglCreatePbufferARB(
											dc, format, w, h, iattribs);
		if (!p->pBuffer)
			return;

		p->dc = globalGL.wglGetPbufferDCARB(p->pBuffer);
		if (!p->dc) {
			globalGL.wglDestroyPbufferARB(p->pBuffer);
			return;
		}

		p->rc = wglCreateContext(p->dc);
		if (!p->rc) {
			globalGL.wglReleasePbufferDCARB(p->pBuffer, p->dc);
			globalGL.wglDestroyPbufferARB(p->pBuffer);
			return;
		}
		...
		globalGL.wglQueryPbufferARB(p->pBuffer,
...
void pbf_makeRead(pbf p)
{
	...
	if (p->isValid && globalGL.supportsMakeCurrentRead) {
		globalGL.wglMakeContextCurrentARB(p->glutWinDc, p->dc, p->glutWinRc);
	}
}
...
void pbf_restore(pbf p)
{
	...
	if (p->isValid) {
		 ...
		 * mode change.
		 */
		globalGL.wglQueryPbufferARB(p->pBuffer, WGL_PBUFFER_LOST_ARB, &lost);
		if (lost) {
